...
# Change pourcentage BM (7/8 * .5) + T*.5 
RUN_BTU = "BTU"
# Change pourcentage BM T/nb_world_claim (t=1 si verite claim et nb_world_claim=nb wld claim com etant la verite)
RUN_BTD = "BTD"
# Change pourcentage BM max(distance hamming) / nb_interp
RUN_BTT = "BTT"
# Change pourcentage BM avg(distance hamming) / nb_interp
RUN_BTQ = "BTQ"
...
# FORMULA = RUN_JA
# Belief Merging Semantic -> methode litt trop forte
# FORMULA = RUN_BM
...
LIST_RUN_NORMAL = [RUN_PRC, RUN_TDP, RUN_PRP, RUN_BTU, RUN_BTD, RUN_BTT, RUN_BTQ, RUN_BIA, RUN_BIF, RUN_BCA, RUN_BCF]
...
def verif(run):
    if FORMULA == RUN_TD:
        ...
        return run in [RUN_BTU, RUN_BTD, RUN_BTT, RUN_BTQ]
    elif FORMULA == RUN_BS:
...
if FORMULA == RUN_BS:
    ...
    PARA_PLOT = [("--o", 'skyblue', r"$Δ^{R_{leximax}}$"), #SF leximax
                 ("--x", 'orange', r"$Δ^{D}$"), # Drastic
                 ("-x", 'plum', r"$Δ^{S,\Sigma}$"), # Symmetric
                 ("-o", 'mediumspringgreen', r"$Δ^{\cap,\Sigma}$"), # Intersect
                 ("-x", 'indianred', r"$SF^{leximin}$"),
                 ("--o", 'mediumslateblue', r"$maxcons$"),
                 ("-", 'orange', r"$maxcard$"),
                 ("--x", 'deepskyblue', r"$SF^{C}_{avg}$"), 
                 (":x", 'cornflowerblue', r"$Δ^{R_{avg}}$"), # SF average 
                 (":x", 'green', r"$SF^{A}_{\Sigma}$")]
    ...
    RUN_METHODS = ["SF", "Sum", "Leximax", "Drastic"]
    PLOT_METHODS = ["SF", "Sum", "Leximax", "Drastic"]
    NAMES_NONORMA = ["SF", "Sum", "Leximax", "Drastic"]
    NAMES = ["SF", "Sum", "Leximax", "Drastic"]
    ORDER = ["SF", "Sum", "Leximax", "Drastic"]
    PLOT_ORDER = ["Drastic", "Sum", "Leximax", "SF"]
    LEGEND_ORDER = ["SF", "Sum", "Leximax", "Drastic"]
    # same order as name
    PARA_PLOT = [("--o", 'blue', r"$SF$"),
                 ("--o", 'black', r"$\sum$"),
                 ("--o", 'red', r"$leximax$"),
                 ("--o", 'orange', r"$drastic$")]
    # Add another method and it will write the results for the sources metrics
    ID_METHODS_SOURCES = [ORDER.index(NAMES[0]), ORDER.index(NAMES[1])]
    #Number of methods we test
    NB_METHODS = len(RUN_METHODS)
    #Number of methods that use the metrics for the sources
    NB_METHODS_SOURCES = len(ID_METHODS_SOURCES)
    ...
    # PLOT_ORDER = ["RDH", "RMSA", "RMCSA", "CMAX", "CSUM", 
    #                "RMWA", "RRA", "SFsum"]
    ...
    # PARA_PLOT = [("--o", 'dodgerblue', r"$R^{\times}$"), # r"$R^{\sum}$" C'EST SUM ICI
    #              ("--o", 'black', r"$R^{leximax}$"),
    #              ("--o", 'red', r"$R^{leximin}$"),
    #              ("--o", 'orange', r"$R^{\times}$"),
    ...
    #              ("-.o", 'crimson', r"$R_{MWA}$"),
    #              (":o", 'plum', r"$R_{RA}$"),
    #              ("-o", 'darkorange', r"$R^{dH,max}$"),
    #              (":o", 'darkgray', r"$\delta^{sum}$"),
    #              ("-o", 'darkslategrey', r"$\delta^{leximax}$"),
    #              ("-.o", 'goldenrod', r"$\delta^{leximin}$")]

    
    PARA_PLOT = [("--o", '#211A44', r"$R^{\times}$"), # r"$R^{\sum}$" C'EST SUM ICI
                 ("--o", 'black', r"$R^{leximax}$"),
                 ("--o", 'red', r"$R^{leximin}$"),
                 ("--o", 'orange', r"$R^{\times}$"),
                 (":o", '#FFD472', r"$R_{MSA}$"),
                 ...
                 (":o", '#F88F52', r"$R_{RA}$"),
                 ("-o", '#E75C38', r"$R^{dH,max}$"),
                 (":o", '#5C5792', r"$\delta^{sum}$"),
                 ("-o", '#1C5588', r"$\delta^{leximax}$"),
                 ("-.o", 'goldenrod', r"$\delta^{leximin}$")]
    ...
    if TD_TEST:
        ...
        PLOT_ORDER = ["Plurality", "Log", "Derive", "Prio"]
        
        LEGEND_ORDER = ["Plurality", "Log", "Derive", "Prio", "BOA"]
        # same order as name
        PARA_PLOT = [("--o", 'cornflowerblue', r"$SF^{A}_{Pl}$"),
                     ("--o", 'dimgrey', r"$SFOP^{A}_{Pl}$"),
                     ("--o", 'lightsalmon', r"$SFUC^{A}_{Pl}$"),
                     ("--o", 'orange', r"$SFAO^{A}_{Pl}$"),
                     ("--o", 'green', r"$SF^{A}_{Bo}$"),]
    else:
        ...
        # PLOT_ORDER = [f"Plurality {NORMA_A}", "Voting"]
...
def PLOT_L(FR):
    if not FR:
        ...
        PLOT_Y = ["Nombre de swaps", "Nombre de swaps (divisé par le max)",
                        ...
                        "Nombre d'itérations moyen", "Nombre d'itérations maximum",
                        ...
                        "Unanime Distance", "Unanime Truth",
                        "Nombre de formules affirmées en moyenne", "Taille moyenne formules", "Précision unanime",
                        "Précision moyenne"]
...
if FORMULA == RUN_TD:
    ...
    ID_METRICS_ACTIVE = {**ID_METRICS_SRC,**ID_METRICS_ALGO,**ID_METRICS_BM}
elif FORMULA == RUN_BS:
