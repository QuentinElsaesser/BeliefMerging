...
class PlotMaxMin():
    ...
    def read_file(self, metric):
        ...
        while line:
            ...
            if line.startswith("\subsection"):
                self.current = 0
                if self.nvalue in constants.LIST_RUN_NORMAL+[constants.RUN_BFA]:#self.nvalue == constants.RUN_PRC or self.nvalue == constants.RUN_TDP or self.nvalue == constants.RUN_PRP or self.nvalue == constants.RUN_BTU or self.nvalue == constants.RUN_BTD or self.nvalue == constants.RUN_BTT or self.nvalue == constants.RUN_BTQ:
                    ...
                    if self.nvalue in constants.LIST_RUN_SRC+[constants.RUN_FCT]:#self.nvalue == constants.RUN_SDR or self.nvalue == constants.RUN_SPR or self.nvalue == constants.RUN_FCT or self.nvalue == constants.RUN_SRC:
                        tmpprc = f"{tmpprc}"
                    else:
    ...
    def plot_one(self, i, d=0, t=-1, height=20, width=10, num_met=0, res=[]):
        ...
        # # UNA
        # mini,maxi,items = self.myplot(x=self.percent, y=res[0][0], idv=possibv[0], i=0, j=0, d=d, t=t, items=items, mini=mini, maxi=maxi, indcol=indcol)
        # indcol += 1 # colonne de la couleur
        # mini,maxi,items = self.myplot(x=self.percent, y=res[3][0], idv=possibv[0], i=0, j=num_met, d=d, t=t, items=items, mini=mini, maxi=maxi, indcol=indcol)
        # indcol += 1
        
        # # MIN
        # mini,maxi,items = self.myplot(x=self.percent, y=res[0][1], idv=possibv[1], i=1, j=0, d=d, t=t, items=items, mini=mini, maxi=maxi, indcol=indcol)
        # indcol += 1 # colonne de la couleur
        # mini,maxi,items = self.myplot(x=self.percent, y=res[3][1], idv=possibv[1], i=1, j=num_met, d=d, t=t, items=items, mini=mini, maxi=maxi, indcol=indcol)
        # indcol += 1
        
        # # MAX
        # mini,maxi,items = self.myplot(x=self.percent, y=res[0][2], idv=possibv[2], i=2, j=0, d=d, t=t, items=items, mini=mini, maxi=maxi, indcol=indcol)
        # indcol += 1 # colonne de la couleur
        # mini,maxi,items = self.myplot(x=self.percent, y=res[3][2], idv=possibv[2], i=2, j=num_met, d=d, t=t, items=items, mini=mini, maxi=maxi, indcol=indcol)
        # indcol += 1
