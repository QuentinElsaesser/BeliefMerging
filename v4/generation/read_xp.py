...
class ReadXP:
    def __init__(self, path, option=1):
        ...
        if self.nvalue == constants.RUN_BTU or self.nvalue == constants.RUN_BTD or self.nvalue == constants.RUN_BTT or self.nvalue == constants.RUN_BTQ:
            self.nb_litt = int(type_gen[-1][3:])
            self.fixed_prc = self.interval[0]
            self.interpretations = list(product([1,0], repeat=self.nb_litt))
        ...
        for i,l in enumerate(lines):            
            ...
            if constants.FORMULA == constants.RUN_BM:
                truth_algo = []
                for n in tmptrue:
                    truth_algo.append(int(n))
            ...
            if constants.FORMULA == constants.RUN_JA:
                ...
                rg = random_graph.randomGraph(voting.Plurality, option, 
                            ...
                            norma=normalization, prior=prior, typeg=self.typeg, min_fs=1,
                            sf=sf, of=of, truth=truth)
            elif constants.FORMULA == constants.RUN_BM:
                rg = random_graph.randomGraph(voting.Plurality, option, 
                              nbo=self.nbo, nbfl=self.nbfl, nbfu=self.nbfu, nbs=self.nbs, 
                            norma=normalization, prior=prior, typeg=self.typeg, min_fs=1,
                            sf=sf, of=of, truth=truth)
                
                rg.G = sfbm.SFbm(rg.G.mat_fs, rg.G.mat_of, voting.Plurality, option, normalization, 
                                 len(rg.G.mat_fs[0]), len(rg.G.mat_fs), truth=truth_algo, Gr=rg.G,
                                 nbl=self.nb_litt, interp=self.interpretations)#, ind_int=self.ind_int)
                # self.ind_int = rg.G.ind_int
    ...
    def find_intv_para(self, trust, nbf, nbo, nbs, indvariance=None, line=None):
        if self.nvalue in constants.LIST_RUN_NORMAL:#self.nvalue == constants.RUN_PRC or self.nvalue == constants.RUN_TDP or self.nvalue == constants.RUN_PRP or self.nvalue == constants.RUN_BTU or self.nvalue == constants.RUN_BTD or self.nvalue == constants.RUN_BTT or self.nvalue == constants.RUN_BTQ:
        ...
        elif self.nvalue in constants.LIST_RUN_SRC:#self.nvalue == constants.RUN_SRC or self.nvalue == constants.RUN_SPR or self.nvalue == constants.RUN_SDR:
