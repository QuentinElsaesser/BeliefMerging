...
class GenerateTDAll:
    def __init__(self, nbs, nbg=1000, nbo=10, nbfu=2, read_xp=False, typeg="TDP", type_generation="n", intvp=['30-34','35-39','40-44','45-49','50-54','55-59','60-64','65-69'], fixed_prc=['50-54']):
        """
    ...
    def find_interv(self, t):
        if self.typeg == constants.RUN_PRC:
            for i in range(len(self.interval)):
                n = self.interval[i]
                tmp = n.split("-")
            ...
            return -1, None
        elif self.typeg == constants.RUN_FCT:
            iv = self.fixed_prc.split("-")
            if int(iv[0]) <= t <= int(iv[1]):
                for i in range(len(self.interval)):
                    intv = self.interval[i]
                    if int(intv) == self.nbfbyo:
                        return self.interval[i], i
            return None, None
        elif self.typeg == constants.RUN_SRC:
            iv = self.fixed_prc.split("-")
            if int(iv[0]) <= t <= int(iv[1]):
                for i in range(len(self.interval)):
                    intv = self.interval[i]
                    if int(intv) == self.nbs:
                        return self.interval[i], i
            return None, None
...
if __name__ == "__main__":
    ...
    generator = GenerateTDAll(nbs=nbs, nbg=nbg, intvp=intv, typeg=typeg, nbo=nbo, nbfu=nbf, 
