...
class PlotBar():
    ...
    def condition(self, ind_method, i):
        ...
        if self.index_m in list(constants.ID_METRICS_SRC.values()):
            if constants.FORMULA == constants.RUN_BM and self.nvalue == constants.RUN_BTU or self.nvalue == constants.RUN_BTD or self.nvalue == constants.RUN_BTT or self.nvalue == constants.RUN_BTQ:
                if ind_method > 1:
                    # Si on est sur les methodes pour le BM et que ce n'est pas les methodes SF on affiche pas sur le metriques sources
                    return False
